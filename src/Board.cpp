...
void Board::moveMonsters(sf::Vector2f playerLoc, float clock_sample)
{
	for (int i = ZERO; i < m_monster.size(); i++)
		m_monster[i]->movement(playerLoc.x, clock_sample, playerLoc.y);
}

void Board::drawMonsters(sf::RenderWindow& win, float clock_sample)
{
	for (int i = ZERO; i < m_monster.size(); i++)
...
bool Board::isOnGround(Player& p)
{
	sf::Vector2f pos = p.getPosition();
	pos.x = (pos.x * SHAPE + TO_CENTER) / SHAPE;
	pos.y = (pos.y * SHAPE + TO_CENTER) / SHAPE;
...
bool Board::isAboveLadder(Player& p)
{
	sf::Vector2f pos = p.getPosition();
...
bool Board::isClimbing(Player& p)
{
	sf::Vector2f pos = p.getPosition();
